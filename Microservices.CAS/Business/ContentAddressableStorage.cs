...
namespace Microservices.CAS.Business
{
	...
	public class ContentAddressableStorage
	{
		...
		public ContentAddressableStorage(string storagePath, CASFileRepository repository, ICacheRepository cache)
		{
		...
		public async ValueTask<string> Store(byte[] content, string name)
		{
		...
		public async ValueTask<byte[]> RetrieveBytes(string name)
		{
			var hash = await _cache.GetByKeyAsync(name);
			if (string.IsNullOrEmpty(hash))
			{
				...
				hash = file.Hash;
				await _cache.SetByKeyAsync(name, hash);
			}
			...
			if (FileManager.FileExists(filePath))
			{
				return await FileManager.ReadFromFile(filePath);
			}
